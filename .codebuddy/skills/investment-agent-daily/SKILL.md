...
# 投资Agent每日策略简报 — 标准工作流 v19.7

> **版本**: v19.7 (2026-08-07) | **北京时间铁律**：第零阶段交易日检测/时间记录改为**强制以北京时间读取**——用 `TZ='Asia/Shanghai' date` 替代裸 `date`，避免系统时区被改（如纽约）导致星期几判断错误、选错模板；原理为切换时区只改显示不改底层UTC时钟，无需联网即可精确；附可选联网校验降级路径。同步更新 core-rules 规则23。起因：用户将系统时区改为纽约，裸 `date` 读到本地时间造成交易日检测偏差
> **版本**: v19.5 (2026-07-02) | **大老板文本铁律**：§九新增"逐段搬运、禁止概括"强约束——大老板消息文本必须逐字完整搬运§1引用块全部模块，严禁重新概括/压缩/改写/删减；致命错误23→24条（+#24大老板文本擅自概括缩水）；known-pitfalls Stage 4 陷阱补第4条。起因：20260702期擅自概括导致内容大量丢失
...
> ⚠️ **北京时间铁律（v19.7）**：交易日检测（判断星期几选模板）与时间记录**必须以北京时间为准**，**严禁使用裸 `date`**。原因：系统时区可能被改为其他时区（如纽约 America/New_York），裸 `date` 读取的是系统本地时间，会导致星期几判断错误、选错模板（致命错误 #7 交易日错误）。
>
> **原理**：切换系统时区只改变显示，底层 UTC 硬件时钟仍准确，因此用 `TZ='Asia/Shanghai'` 强制指定即可 100% 得到准确北京时间——无需联网、无外部依赖、不会失败。

```bash
# 【交易日检测基准】强制以北京时间（Asia/Shanghai）读取，忽略系统本地时区设置
TZ='Asia/Shanghai' date "+%A %Y-%m-%d %H:%M:%S"        # 北京时间——星期几判断以此为准
REPORT_START_TIME=$(TZ='Asia/Shanghai' date "+%H%M")   # 北京时间时刻记录

# 【自查】对比一下系统本地时间，若两者星期几不同，说明系统时区确实被改，必须以上面北京时间为准
date "+(本地时区参考) %A %Y-%m-%d %H:%M:%S %Z"
```

根据**北京时间**的星期几选择对应模板（周六～周日→不出 / 周一→特殊 / 周二~周五→标准）。

> **可选联网校验（降级备用，非必需）**：若怀疑本机UTC 时钟本身被手动改过（而非仅切换时区），可联网获取北京时间做二次校验，例如：
> ```bash
> # 方式A：读取权威站点 HTTP 响应头的 Date（GMT/UTC），本地按需换算为北京时间（+8h）
> curl -sI https://www.baidu.com | grep -i '^date:'
> # 方式B：淘宝时间接口（返回毫秒时间戳，UTC 基准）
> curl -s 'https://acs.m.taobao.com/gw/mtop.common.getTimestamp/'
> ```
> 联网仅作校验用途；**默认方案始终为 `TZ='Asia/Shanghai' date`**，联网失败不阻塞流程。
...
> ⚠️ **强制提醒（v19.5 升级）**：输出大老板消息文本前，**必须重新读取 [report-format-guide.md § 九](references/report-format-guide.md)，逐字段对照模板填充，禁止凭记忆输出**。固定开头"老板好～今日二级策略简报结论如下："一字不改，📌 emoji不可省略。
>
> 🚫 **禁止概括铁律（v19.5 — 致命错误#24）**：大老板文本 = §1引用块的**逐段逐字完整搬运**（标题/事件链/全球资产反应/市场分化/正负向锚/📌结论全部模块），**仅做"去markdown语法"的机械清洗**（去`>`、`**`、`🚨`）。**严禁重新概括、压缩、改写、删减任何模块或任何一条 bullet**。凡§1引用块里有的，大老板文本必须一字不少地出现。输出前自检：§1引用块有 N 个模块/bullet，大老板文本是否也有 N 个且内容一字不差？若发现自己在"用更短的话重说"，即为违规，必须回退重来。起因：20260702期擅自概括缩水导致交付事故。
...
| **24** | **大老板消息文本擅自概括/缩水改写（输出了但未逐段完整搬运§1引用块，而是重新概括/压缩/删减模块或bullet，导致事件链/全球资产反应/市场分化/正负向锚/持仓策略内容丢失）** | **v19.5** |
...
| 文件 | 内容 | 用途 |
|------|------|------|
| [core-rules.md](references/core-rules.md) | 32条核心规则完整版 + 致命错误清单 + 终审清单 + 禁用词表 | 规则参考 |
...
| 版本 | 日期 | 核心变更 |
|------|------|---------|
| **v19.7** | 2026-08-07 | **北京时间铁律（交易日检测时区修复）**：①第零阶段时间读取由裸 `date` 改为 `TZ='Asia/Shanghai' date`——星期几判断/时间记录强制以北京时间为准，忽略系统本地时区；②新增"北京时间铁律"说明块（原理：切时区只改显示不改底层UTC时钟，无需联网即精确）+ 本地时区自查对比命令 + 可选联网校验降级路径（百度HTTP Date头/淘宝时间接口）；③同步更新 core-rules 规则23（`date "+%A"`→`TZ='Asia/Shanghai' date "+%A"`）。起因：用户将系统时区改为纽约(America/New_York)，裸 `date` 读到本地时间导致交易日检测偏差、可能选错模板 |
| **v19.5** | 2026-07-02 | **大老板文本防线加固 + 全局版本同步**：①report-format-guide §九新增"逐段搬运、禁止概括"铁律+输出前自检项——大老板消息文本必须逐字完整搬运§1引用块全部模块，严禁概括/压缩/改写/删减；②致命错误23→24条（+#24大老板文本擅自概括缩水）；③SKILL.md第四阶段强制提醒升级"禁止概括铁律"；④known-pitfalls Stage 4 交付格式陷阱补第4条（内容缩水）；⑤修正知识库索引"core-rules 30条"→"32条"（v19.4漏改）；⑥report-format-guide/core-rules/known-pitfalls 版本头同步。起因：20260702期擅自概括导致事件链/全球资产反应/市场分化/正负向锚/持仓策略大量丢失 |
