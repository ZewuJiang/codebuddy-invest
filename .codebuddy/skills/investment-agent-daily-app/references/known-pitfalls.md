...
> **用途**：投研鸭小程序数据生产过程中的已知堵点与应对方案（共64条活跃，1条新归档）。
...
## 并行采集与 Context 压缩堵点（v11.0 新增）

| # | 堵点 | 降级路径 | 频率 |
|---|------|---------|------|
| 57 | **并行组数据竞争**：P1-P4 并行发出工具调用时，同一行情源（如 Google Finance）可能对短时间大量请求限流/403 | → P2 组内 web_fetch 若遇 403，按 data-source-priority.md 降级到备选源；P1/P3/P4 不受影响（使用 web_search 不同源）。**同一组内不重试同一 URL**，直接切备选 | 中 |
| 58 | **Context 压缩丢失关键字段**：AI 在提取最小字段集时遗漏了某个关键数据（如 change 漏提取、price 格式错误），导致 Phase 2 JSON 生成时字段为空 | → **强制规则**：提取后立即逐字段核对 data-collection-sop.md §0.9 的字段表，缺失的字段**立即重新 web_fetch** 补取。Phase 1.5 门禁会拦截缺失数据 | 高 |
| 59 | **串行组 S1 等待超时**：P1-P4 某组因网络问题长时间未返回，S1 无法启动，导致总采集时间反而超过串行方案 | → **超时保护**：单个并行组如果超过 3 分钟（约 60 次工具调用）未完成，记录已完成的结果，放弃未完成的请求，进入 S1。Phase 1.5 门禁拦截缺失数据 | 低 |
| 60 | **内联自校验橡皮图章**：Generator-Verifier 循环中 AI 只走过场不认真检查，FATAL 项实际存在但内联校验报告"全部通过"，到 Phase 3 validate.py 才发现 | → **防治措施**：inline-verifier-rules.md 定义了具体的检测方法（不是模糊的"检查一下"），每项都有明确的判定条件和阈值。AI 必须逐项执行而非笼统回答。Phase 3 validate.py 是终极门禁，内联校验无法绕过它 | 中 |

## v11.0 试运行新发现堵点

| # | 堵点 | 降级路径 | 频率 |
|---|------|---------|------|
| 61 | **validate.py 检测根目录旧文件**：SYNC_DIR 指向 `miniapp_sync/`（无日期），validate.py 检测根目录文件而非日期子目录，若根目录为旧文件则产生误报 WARN。**判断方法**：WARN 报告中的具体文本与当日 JSON 不一致 → 旧文件误报，`--skip-warn` 安全跳过 | 检查 WARN 报告文本是否与当日JSON一致，不一致则为误报 | 高频 |
...
| 64 | P0 路径不一致 — AI 写日期子目录 vs 工具链读根目录 | **v6.1 已永久修复**：run_daily.sh 新增第-1步自动同步日期子目录→根目录 |

</details>

---

> v5.5 — 2026-04-21 | **规范健康度优化**：堵点 #64（P0路径不一致）已被 run_daily.sh v6.1 永久修复，移入归档区；活跃堵点数 65→64。
> v5.4 — 2026-04-21 | **涨跌符号错误 + 数据版本分裂**：新增堵点 #65，活跃堵点数 64→65。
> v5.3 — 2026-04-21 | **P0 路径Bug修复**：新增堵点 #64（已归档），run_daily.sh 升级至 v6.1。
> v5.0 及更早版本 | 详见 git log 或 CHANGELOG.md。
