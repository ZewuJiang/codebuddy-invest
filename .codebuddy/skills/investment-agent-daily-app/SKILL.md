...
# 投研鸭小程序数据生产 — 标准工作流 v10.5

> **版本**: v10.5 (2026-04-09 22:27) — Harness v10.5 门槛全面收紧：数据准确性 > 执行速度
> **主控文档**：本文件为精炼主控（≤280行），详细规则通过引用按需加载。
> **核心改造**：①validate.py v5.5（54项校验/10项FATAL/零容忍门禁）②data-collection-sop.md v2.1（4级搜索重试SOP）③产出质量宪章（数据准确+内容严谨+前端高质量）
...
| 已知堵点条数 | 56（活跃） | 2026-04-09 |
| trafficLights 阈值版本 | v4.8 | 2026-04-05 |
| 上次月度审计 | 未执行 | — |
| 连续零事故天数 | 0 | 2026-04-09 |
| 回归门禁条数 | 9 (R1-R9) + **10项FATAL** (V6/V39/V40/V43/V44/V45/V46/R2/R3/R9) | 2026-04-09 |
| 持仓缓存版本 | v1.1 (2025Q4 13F 已修正) | 2026-04-08 |
| **validate.py 版本** | **v5.5 (54项 含10项FATAL)** | **2026-04-09** |
...
| **data-collection-sop.md** | **v2.1 (含§0.5/§0.6/§0.7 数据防护)** | **2026-04-09** |
| **质量基准** | **4/9 22:27 版 (53/54 PASS, 0 FATAL)** | **2026-04-09** |

### v10.5 Changelog（2026-04-09 22:27）
- **🛡️ Harness v10.5 — 门槛全面收紧（数据准确性 > 执行速度）**：
  - **validate.py v5.4→v5.5**：V6(WARN→FATAL,≤5%) + V40(WARN→FATAL) + V44(零容忍任何零值) + V45(30%阈值) + V46[FATAL](chartData零值) + V47[WARN](平线检测)，FATAL项5→10，校验项52→54
  - **data-collection-sop.md v2.0→v2.1**：§0.5 price与sparkline同源规则 + §0.6 sparkline禁止零值填充 + §0.7 质量门禁与4级搜索重试SOP
  - **核心原则**：宁可多搜几轮花多几分钟，也不能让错误数据发布

### v10.4 Changelog（2026-04-09 21:53）
- **🔧 Harness v10.4 — 结构性数据防护（15个标的price错误+11个sparkline全0修复）**：
  - 修正 watchlist.json 15个标的 price（智谱/MiniMax/AVGO/TSM/ASML/AMD 等）+ 补全全部32个标的 7日/30日涨跌
  - 修正 markets.json 11个 sparkline（VIX/META/日经/KOSPI/黄金/布伦特/DXY/10Y美债/CNH/BTC/ETH）
  - validate.py v5.3→v5.4：+V44[FATAL](sparkline零值) +V45[FATAL](数量级一致)

### v10.3 Changelog（2026-04-08 23:42）
- **🛡️ 4/8 基准质量巩固 — sparkline/price 全自动对齐 + 产出质量宪章**：
  - auto_compute.py v2.0→v3.0 + validate.py v5.1→v5.3(+V43 FATAL) + 产出质量宪章
...
| **质量基线** | validate.py 54项校验, **0 FATAL**（10项FATAL全通过）方可上传。WARN ≤3 |
