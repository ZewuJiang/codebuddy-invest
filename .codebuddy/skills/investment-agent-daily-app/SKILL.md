...
# 投研鸭小程序数据生产 — 标准工作流 v11.1

> **版本**: v11.1 (2026-04-21 12:30) — P0路径Bug修复 + asOf括号处理 + JSON双引号防治 + 每日操作Checklist
> **上一版本**: v11.0 (2026-04-20 20:00) — Phase 1 并行采集 + Context 压缩 + References 分层加载 + Generator-Verifier 内联自校验
> **主控文档**：本文件为精炼主控，详细规则通过引用按需加载（三批分层：L1/L2/L3）。
> **核心改造**：①Phase 1 采集 4 组并行（时间减少 60-70%）②web_fetch 后立即提取最小字段集丢弃 HTML（context ~76k→~35k）③References 三批按需加载 ④Phase 2 写完即检 Generator-Verifier 内联自校验（14/17 项 FATAL 前置拦截）
...
| **validate.py 版本** | **v5.6 (54项 含17项FATAL)** | **2026-04-13** |
| **auto_compute.py 版本** | **v3.0 (15类字段自动计算)** | **2026-04-08** |
| **run_daily.sh 版本** | **v6.1（新增第-1步：日期子目录→根目录同步）** | **2026-04-21** |
...
- **🔧 v11.1 — Harness 深度体检全量修复**：
  - **Skill规范修复**：大标题 v11.0→v11.1；优先级声明章节标题补全（修复孤立表格）；引用文件索引中 data-collection-sop 描述更新至 v3.1；Phase 1 采集SOP链接描述对齐 v3.1
  - **Checklist J3升级**：目视检查→机器可执行 `python3 json.load` 命令，消除 AI 橡皮图章漏洞
  - **前端代码修复**：radar.js 版本注释 v7.0→v7.1；briefing.wxml 版本注释 v7.0→v7.1；核心判断默认展开（expanded false→true）；参考源链接显示 domain 而非完整 URL（briefing.js 增加域名提取逻辑）；dataTime 移除无效 split('/')（radar.js）；watchlist.wxml 精简标的数量提示
  - **color.js 语义修复**：getTrendInfo hold 标签 "中性"→"观望"

### v11.0 Changelog（2026-04-20 20:00）
- **🏗️ v11.0 — 局部并行 + 单 Agent 主控 + Context 压缩 + Generator-Verifier 四大架构升级**：
  - **Phase 1 并行采集**：10 个串行批次改为 4 组并行（P1媒体/P2行情/P3亚太大宗/P4基金）+ 1 组串行（S1依赖P2），采集时间预期减少 60-70%
  - **Context 压缩铁律**：新增 [data-collection-sop.md §0.9](references/data-collection-sop.md) 定义 11 个批次精确最小字段集提取规范，web_fetch 后立即提取→丢弃 HTML，Phase 1 上下文从 ~76k 压缩至 ~35k
  - **References 分层加载**：从一次性全加载改为三批按需（L1 Phase 0 核心 / L2 Phase 1 前采集知识库 / L3 Phase 2 前 Schema+自校验）
  - **Generator-Verifier 内联自校验**：新增 [inline-verifier-rules.md](references/inline-verifier-rules.md)，Phase 2 每个 JSON 写完后立即校验（14/17 项 FATAL 可内联检测），最多 2 次修复重试，避免 Phase 3 FATAL 后整体重来
  - data-collection-sop.md v2.1→v3.0（+§0.8 并行分组 +§0.9 最小字段集）
  - known-pitfalls.md v4.5→v5.0（+#57-#60 并行采集堵点）
  - 工具链零改动（validate.py/auto_compute.py/run_daily.sh 全部不变）
...
> 3. **第0.5步：validate.py v5.6 自动化校验（54项，FATAL级不可绕过）**
> 4. 第1步：sparkline/chartData 补全（软依赖）
> 5. 第2步：上传云数据库
> 6. 第3步：同步公开API

### Phase 3.5：语音播报生成（🔴 FATAL 级强制，跳过 = 阻断上传）
> validate.py V35 为 **FATAL 级**，audioUrl 为空将直接阻断上传。
...
**14/17 项 FATAL 可在 Phase 2 内联检测**（写完即检，避免 Phase 3 整体重来）：
V6, V24, V38, V39, V40, V41, V42, V43, V44, V45, V46, R1, R2, R3

**3 项 FATAL 不可内联**（必须等 Phase 3 脚本执行）：
R9（需 holdings-cache.json）、V35（需 TTS 执行后）、V_TL（需 auto_compute 执行后）
...
| [known-pitfalls.md](references/known-pitfalls.md) | 已知堵点60条+降级路径 | **L4** Phase 4/5 |
