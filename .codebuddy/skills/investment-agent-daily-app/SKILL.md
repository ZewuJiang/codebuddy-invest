...
#### 4.3 同步公开 API（AnyDev 服务器 — v7.2 新增）

> **目的**：将4个JSON + 渲染后的 briefing.md 同步到 AnyDev 公网服务器，供 ClawHub Skill 消费。
> **触发时机**：每次 Heavy 和 Refresh 模式均执行（无论是否生成音频）。
> **Refresh 模式同样执行**：行情每4小时刷新，briefing.md 需同步更新。
...
```
此脚本会自动：
- 调用 `render_briefing.py` 将4个JSON渲染为 `briefing.md`
- 复制文件到 `touyanduck-api/api/latest/`
- 生成 `meta.json`
- 打包为 `/tmp/touyanduck-api-latest.tar.gz`

**步骤2：上传到 AnyDev 服务器**（调用 AnyDev Integration）
...
**步骤3：远程解压**（调用 AnyDev webshell）

```bash
su -c 'tar -xzf /data/anydev_upload/touyanduck-api-latest.tar.gz -C /data/touyanduck-api/api/latest/'
```

**步骤4：验证**（调用 AnyDev webshell）

```bash
curl -s -o /dev/null -w '%{http_code}' http://localhost:8080/touyanduck
# 预期返回 200
```

**交付确认补充**：
```
🌐 公开API同步：http://21.214.207.96:8080/touyanduck ✅
🦆 ClawHub Skill 可读取最新 briefing.md
```

> **失败处理**：若 AnyDev 上传失败，不影响小程序数据（微信云数据库已上传成功）。
> 可事后手动重跑：`bash sync_to_edgeone.sh {YYYY-MM-DD}` + 手动调用 AnyDev Integration 上传。
