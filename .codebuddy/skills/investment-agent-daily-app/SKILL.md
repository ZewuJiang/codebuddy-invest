...
# 投研鸭小程序数据生产 — 标准工作流 v7.2

> **版本**: v7.4 (2026-04-07 21:42)
...
| 已知堵点条数 | 36 | 2026-04-07 |
| trafficLights 阈值版本 | v4.8 | 2026-04-05 |
| 上次月度审计 | 未执行 | — |
| 连续零事故天数 | 1 | 2026-04-07 |

### v7.4 Changelog（2026-04-07 21:42）
- **第4.3阶段升级：GitHub Pages 作为主要公网端点**：
  - 主端点改为 `https://zewujiang.github.io/touyanduck-api/briefing.md`（全球可访问，HTTPS，无需备案）
  - AnyDev 降级为备用端点（仅中国大陆可访问）
  - 同步流程：sync_to_edgeone.sh → 复制到 edgeone-deploy → git push → GitHub Pages 自动更新
  - 仓库地址：`github.com/ZewuJiang/touyanduck-api`

### v7.3 Changelog（2026-04-07 20:40）
- **新增第4.3阶段：AnyDev 公开API同步（全自动化闭环）**：
  - Heavy 和 Refresh 模式均在交付确认后自动执行 AnyDev 同步
  - 流程：`sync_to_edgeone.sh`（渲染+打包）→ AnyDev file_upload → webshell 解压 → 验证 200
  - 同步内容：4个JSON + briefing.md（渲染后的Markdown简报）
  - 失败不影响小程序，可事后手动补跑

### v7.2 Changelog（2026-04-07 18:30）
- **工程化改造（对标 Iran Briefing skill）**：
  - **sync_to_edgeone.sh v1.0→v2.0**：从本地复制升级为完整远程同步（本地打包 tar.gz → AnyDev file_upload → webshell 解压）
  - **Nginx 替代 Python HTTP Server**：CORS 头支持（`Access-Control-Allow-Origin: *`）+ `Cache-Control: no-cache` + 健康检查端点 `/health` + systemd 自启动
  - **refreshInterval 统一修正**：从"每4小时更新"→"每工作日更新（日内可手动刷新）"，消除承诺失配（影响 meta.json / briefing.json / SKILL.md / index.html 共6处）
  - **json-schema.md v4.2→v4.5**：`coreEvent.chain[]` 新增 `source_count` 可选字段（多源交叉验证标记：1=单源/2-3=多源确认/4+=广泛报道）
  - **ClawHub SKILL.md**：数据获取策略从"全量拉取"重构为"按需钻取"（对标 Iran Briefing: "先拉简报，按需钻取"），首次只拉 meta+briefing（~15KB），追问时按需拉取 markets/watchlist/radar

### v7.1 Changelog（2026-04-07 13:00）
- **P0 残留 Bug 修复（3处）**：
  - data-collection-sop.md 标题版本号 v1.8→v1.9（第1/6行未同步的遗留问题）
  - RULE SEVEN 表格中 Refresh 搜索基线从错误的 `≥4次web_search+0次web_fetch` 修正为正确的 `≥2次web_search+4次web_fetch`
  - known-pitfalls.md 堵点总数从 35 修正为实际 36 条（编号至#36）+ 规范健康度快照同步
- **P1 产出质量提升**：
  - refresh-mode.md v1.0→v1.1：`refreshInterval` Refresh模式写"每4小时更新" / `sentimentScore` 微调量化规则（VIX±3点/SPX±1.5%/突发事件） / `_meta.refreshCount` 可选字段 / 四个JSON_meta边界表统一补全
  - json-schema.md v4.2→v4.4：refreshInterval Refresh 模式说明 + _meta.refreshCount 字段定义
  - data-collection-sop.md R1 批次 M7 默认策略明确化（指数+VIX必须web_fetch，M7分时段策略）
  - known-pitfalls.md v3.1→v3.2

### v7.0 Changelog（2026-04-07 12:00）
- **新增 Refresh 模式（每4小时增量刷新）**：
  - 三档内容引擎升级为**四档**：Heavy / Refresh / Weekend / Live(TODO)
  - 新建 `references/refresh-mode.md` v1.0：完整 Refresh 模式规范（设计哲学/精简采集批次R0-R3/字段刷新保留边界表/4个JSON产出规则/精简终审7项/工作流/关键约束）
  - 模式路由新增 Refresh 分支：当天 miniapp_sync/ 已有4个JSON → Refresh；否则 → Heavy
  - 降级安全网：Refresh 启动后若基准JSON不完整，自动降级为 Heavy
  - `_meta.sourceType` 枚举新增 `refresh_update`
  - Refresh 精简采集批次：R0（快扫头条）+ R1（美股行情）+ R2（亚太/大宗/加密）+ R3（异动信号），总搜索量 5-6 次（Heavy 的 1/10）
  - Refresh 精简终审：7项门禁（price来源追溯/数据类型/trafficLights一致性/riskScore公式/枚举合规/保留字段未篡改/JSON语法）
  - RULE SEVEN 新增 Refresh 豁免：Refresh 不执行聪明钱5层搜索
  - 搜索执行日志达标基线新增：Refresh ≥ 2次 web_search + 4次 web_fetch
  - 语音播报仅在 Heavy 模式后生成，Refresh 跳过第3.5阶段
  - data-collection-sop.md v1.8→v1.9：新增第九章 Refresh 精简采集批次定义；R1 批次 M7 默认策略明确化
  - known-pitfalls.md v3.0→v3.1：新增堵点 #34/#35/#36（Refresh 相关）
...
| **RULE SEVEN** | **聪明钱搜索广覆盖+产出精选铁律（v4.8）**：<br>● **搜索范围必须广**：每次执行必须走完5层搜索流程（广播→一级核心定向→ARK web_fetch→策略师→知识库联动），最低搜索次数：Heavy≥10次web_search+1次web_fetch；Weekend≥8次+1次；13F模式≥14次+1次<br>● **Refresh 模式豁免（v7.0）**：Refresh 不执行聪明钱5层搜索（Batch 4 完全跳过），聪明钱数据保留基准 JSON。Refresh 搜索基线独立：≥2次web_search+4次web_fetch<br>● **产出精选不凑数**：smartMoneyDetail 不设最低条数，只选真正有料的。**宁少而精，不多而杂**<br>● **深挖强制**：web_fetch≥2次（ARK 1次+高价值来源≥1次）<br>● **精选标准**：有具体动作/数字 > 有明确方向判断 > 时效性近。泛泛空话淘汰<br>● 详细搜索流程 → [fund-universe.md §七](references/fund-universe.md) |
...
> **v7.0 升级**：从「一天一次全量」升级为「每4小时增量刷新」，大老板每次打开都看到最新行情。

### 四档模式路由表

| 时机 | 模式 | 采集量 | 内容侧重 | `_meta.sourceType` |
|------|------|--------|---------|-------------------|
| 周一（盘后/当天首次） | **Heavy + 周一特别版** | 全量6批次+Batch A | 行情+分析+上周总结+本周展望 | `heavy_analysis` |
| 周二～周五（当天首次） | **Heavy** | 全量6批次+Batch A | 行情+分析+建议 | `heavy_analysis` |
| **工作日后续每4小时** | **Refresh**（v7.0 新增） | **精简4批次（R0~R3）** | **刷新行情+红绿灯+异动信号，保留深度分析** | `refresh_update` |
...
定时任务触发
  → Step 1: 今天是周几？
    ...
    → 周一~周五 → Step 2: 是否为美股休市日？（Good Friday / MLK Day 等）
      → 休市日 → Weekend 模式
      → 非休市日 → Step 3: 当天 miniapp_sync/ 目录下是否已有 4 个 JSON？
        → 否 → Heavy 模式（当天首次执行，全量采集+生成）
        → 是 → Step 4: 基准 JSON 的 _meta.sourceType？
          → "heavy_analysis" 或 "refresh_update" → Refresh 模式（增量刷新）
          → "weekend_insight" → Heavy 模式（周一首次应覆盖周末版）
          → 基准 JSON 不完整 → Heavy 模式（降级安全网）
```
...
### Refresh 模式内容规范（v7.0 新增 — 详细规范已外迁）

> **详细规范** → [refresh-mode.md](references/refresh-mode.md)（含精简采集批次R0-R3、字段刷新/保留边界表、4个JSON产出规则、精简终审7项、工作流、关键约束）
>
> **一句话概要**：工作日后续每4小时读取当天 Heavy 版4个JSON文件作为基准，**更新行情数据（price/change/trafficLights/riskScore等），保留深度分析内容不变**。`_meta.sourceType` = `"refresh_update"`。
>
> **关键约束**：必须有当天 Heavy 版基准、RULE ZERO 不降级、保留字段零篡改、不生成语音播报。
>
> **与 Weekend 模式的互补关系**：Weekend = 保留行情、更新分析；**Refresh = 更新行情、保留分析**。
...
- **周一～周五 + 当天已有4个JSON** → **Refresh 模式**（详见 [refresh-mode.md](references/refresh-mode.md)）
  - 读取当天 Heavy 版 4 个 JSON 文件作为基准
  - 精简采集（R0~R3）→ 更新行情/红绿灯/异动信号 → 保留深度分析 → 上传
  - 降级安全网：基准不完整 → 自动降级为 Heavy
...
| O3 | 所有4个JSON: _meta（建议） | sourceType+generatedAt(ISO8601格式)+skillVersion | sourceType 按模式填写（Heavy→`heavy_analysis` / Weekend→`weekend_insight` / Refresh→`refresh_update`）；generatedAt 为执行时间（+08:00）；skillVersion 为当前 SKILL.md 顶部版本号 |
...
> **此阶段为强制性门禁，不通过则禁止上传。**
> **Refresh 模式使用精简终审（7项）**：详见 [refresh-mode.md §精简终审门禁](references/refresh-mode.md)。Refresh 只校验被更新的字段（price来源追溯/数据类型/trafficLights/riskScore公式/枚举合规/保留字段未篡改/JSON语法），跳过深度内容门禁（因内容未被修改）。
> **Heavy/Weekend 模式使用完整终审（14项+Top10）**：如下所述。
...
> **⚠️ Refresh 模式跳过此阶段**：语音内容是深度分析，一天一次由 Heavy 负责。Refresh 保留 Heavy 版的 voiceText/audioUrl/audioFile 不变。
> 前提：4个 JSON 文件已上传成功。
...
达标基线：Heavy≥10+1 / Weekend≥8+1 / **Refresh≥2+4** / 13F≥14+1（RULE SEVEN）
```
...
#### 4.3 同步公开 API（GitHub Pages 主 + AnyDev 备 — v7.3）

> **目的**：将4个JSON + 渲染后的 briefing.md 同步到公网，供 ClawHub Skill 全球访问。
> **主要公网端点**：GitHub Pages（全球可访问，HTTPS，无需备案）
> **备用端点**：AnyDev 服务器（内网，仅中国大陆可访问）

**执行步骤**：

**步骤1：本地渲染 + 打包**
```bash
bash /Users/zewujiang/Desktop/AICo/codebuddy-invest/.codebuddy/skills/investment-agent-daily-app/scripts/sync_to_edgeone.sh {YYYY-MM-DD}
```

**步骤2：同步到 GitHub Pages（主要公网端点）**
```bash
# 复制最新文件到 edgeone-deploy 目录
cp /Users/zewujiang/Desktop/AICo/codebuddy-invest/touyanduck-api/api/latest/*.json \
   /Users/zewujiang/Desktop/AICo/codebuddy-invest/touyanduck-api/api/latest/briefing.md \
   /Users/zewujiang/Desktop/AICo/codebuddy-invest/touyanduck-api/edgeone-deploy/

# Git push 到 GitHub（自动触发 GitHub Pages 更新）
cd /Users/zewujiang/Desktop/AICo/codebuddy-invest/touyanduck-api/edgeone-deploy
git add -A
git commit -m "data: update briefing {YYYY-MM-DD}"
git push origin main
```

**步骤3：同步到 AnyDev（备用端点，供中国大陆访问）**
```
调用 AnyDev file_upload：
  filePath: /tmp/touyanduck-api-latest.tar.gz
  destinationPath: /data/anydev_upload/touyanduck-api-latest.tar.gz

调用 AnyDev webshell：
  su -c 'tar -xzf /data/anydev_upload/touyanduck-api-latest.tar.gz -C /data/touyanduck-api/api/latest/'
```

**步骤4：验证**
```bash
# 验证 GitHub Pages（主端点）
curl -s -o /dev/null -w '%{http_code}' https://zewujiang.github.io/touyanduck-api/briefing.md
# 预期返回 200
```

**交付确认补充**：
```
🌐 GitHub Pages 主端点: https://zewujiang.github.io/touyanduck-api/briefing.md ✅
🌐 AnyDev 备用端点: http://21.214.207.96:8080/touyanduck（仅中国大陆）
🦆 ClawHub Skill 全球可访问
```

> **失败处理**：若 GitHub push 失败，不影响小程序（微信云数据库已上传）。可事后手动重跑 git push。
...
> **Refresh 模式使用精简版复盘**：仅检查前2项（堵点+数据一致性），跳过 trafficLights 组合评估（一天一次由 Heavy 负责）。详见 [refresh-mode.md §执行复盘](references/refresh-mode.md)。
...
| 16 | 可选字段枚举值越界 | `predictions[].source` 填了非允许值（仅限 Polymarket/Kalshi/CME FedWatch）；`_meta.sourceType` 填了非枚举值（仅限 `heavy_analysis`/`weekend_insight`/`refresh_update`） |
...
| [refresh-mode.md](references/refresh-mode.md) | **Refresh 模式完整规范 v1.1**（精简采集R0-R3+字段边界表+精简终审+工作流+sentimentScore微调规则+refreshCount） | 工作日增量刷新执行 |
