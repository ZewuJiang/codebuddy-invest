...
# 投研鸭小程序数据生产 — 标准工作流 v10.6

> **版本**: v10.6 (2026-04-13 11:05) — Harness v10.6：FATAL 项全面升级，目标每次全部通过
> **主控文档**：本文件为精炼主控（≤280行），详细规则通过引用按需加载。
> **核心改造**：①validate.py v5.6（54项校验/**17项FATAL**/零容忍门禁）②语音播报强制FATAL ③前端渲染安全全面FATAL化
...
### v10.6 Changelog（2026-04-13 11:05）
- **🛡️ Harness v10.6 — FATAL 项全面升级（目标：每次全部通过，零 WARN）**：
  - **validate.py v5.5→v5.6**：FATAL 项 10→17（新增 V24/V35/V38/V41/V42/R1/V_TL）
  - **V35 WARN→FATAL**：audioUrl 为空 = 语音播报失效（核心功能，必须有）
  - **V38 WARN→FATAL**：sparkline 趋势与 change 方向矛盾 = 数据错误（不是轻微警告）
  - **V41 WARN→FATAL**：globalReaction value 超长/含括号 = 前端布局溢出
  - **V42 WARN→FATAL**：generatedAt 为空 = 前端时间显示异常
  - **V24 WARN→FATAL**：Markdown 残留 = 前端直接渲染乱码
  - **R1  WARN→FATAL**：topHoldings < 3 = 聪明钱核心展示严重不完整
...
### 第3.5阶段：语音播报生成（🔴 FATAL 级强制，跳过 = 阻断上传）
> **v10.6 升级**：validate.py V35 已升级为 **FATAL 级**，audioUrl 为空将直接阻断上传，不可 --skip-warn 绕过。
